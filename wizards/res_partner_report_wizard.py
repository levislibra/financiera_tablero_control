...
class ResPartnerReportWizard(models.TransientModel):
	...
	def generate_excel(self, records):
		...
		sheet.write(0, 5, 'Prestamos activos')
		sheet.write(0, 6, 'Prestamos cobrados')
		sheet.write(0, 7, 'Cuotas activas')
		sheet.write(0, 8, 'Cuotas cobradas')
		sheet.write(0, 9, 'Cuotas sin mora')
		sheet.write(0, 10, 'Cuotas en preventiva')
		sheet.write(0, 11, 'Cuotas en mora temprana')
		sheet.write(0, 12, 'Cuotas en mora media')
		sheet.write(0, 13, 'Cuotas en mora tardia')
		...
		for partner_id in records:
			...
			sheet.write(row, 5, partner_id.alerta_prestamos_activos)
			sheet.write(row, 6, partner_id.alerta_prestamos_cobrados)
			sheet.write(row, 7, partner_id.alerta_cuotas_activas)
			sheet.write(row, 8, partner_id.alerta_cuotas_cobradas)
			sheet.write(row, 9, partner_id.alerta_cuotas_normal)
			sheet.write(row, 10, partner_id.alerta_cuotas_preventivas)
			sheet.write(row, 11, partner_id.alerta_cuotas_temprana)
			sheet.write(row, 12, partner_id.alerta_cuotas_media)
			sheet.write(row, 13, partner_id.alerta_cuotas_tardia)
